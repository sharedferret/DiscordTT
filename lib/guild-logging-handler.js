...
const handleGuildMemberAdd = (embed, data) => {
  embed.setAuthor(`Joined ${data.member.guild.name}`, data.member.user.avatarURL(256));
  embed.setThumbnail(data.member.user.avatarURL(256));
...
const handleGuildMemberRemove = (embed, data) => {
  embed.setAuthor(`Left ${data.member.guild.name}`, data.member.user.avatarURL(256));
  embed.setThumbnail(data.member.user.avatarURL(256));
  embed.addField('User', `<@${data.member.user.id}>`);
    ...
    switch(eventName) {
      ...
      case 'messageDelete':
        if (!data.message.author.bot && data.message.channel.id !== settings.logs.logChannel) {
          ...
          embed.setAuthor('Message Deleted', data.message.author.avatarURL(256));
          embed.setThumbnail(data.message.author.avatarURL(256));
      ...
      case 'messageUpdate':
        ...
        if (!data.newMessage.author.bot && data.newMessage.channel.id !== settings.logs.logChannel && data.newMessage.content != data.oldMessage.content) {
          embed.setAuthor('Message Edited', data.newMessage.author.avatarURL(256));
          embed.setThumbnail(data.newMessage.author.avatarURL(256));
          embed.setColor('GOLD');
      ...
      case 'guildBanAdd':
        ...
        embed.setThumbnail(data.user.avatarURL(256));
        embed.setColor('RED');
        embed.addField('User', `<@${data.user.id}>`, true);
        break;
      default:
        return;
        break;
