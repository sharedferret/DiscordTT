    ...
    db.all('select p.id AS playlistId, p.name AS playlistName, COUNT(q.id) as songCount, CASE WHEN p.id = u.activePlaylistId THEN \"true\" ELSE \"false\" END AS active FROM Playlist p LEFT JOIN UserQueue q ON p.id = q.playlistId LEFT JOIN User u ON p.userId = u.id WHERE u.id = ? GROUP BY p.id ORDER BY p.id ASC',
      ...
      function(err, rows) {
        if (rows && rows.length > 0) {
          ...
          embed.setAuthor(message.author.username + '\'s Playlists', message.author.avatarURL(256));
  ...
  db.serialize(function() {
    db.get('INSERT INTO Playlist (id, userId, name, metadata) VALUES (?, ?, ?, ?)',
      [
        ...
        // Make it the user's active playlist
        db.run('UPDATE User SET activePlaylistId = ? WHERE id = ?', [ newPlaylistId, message.author.id ]);
    ...
    db.get('SELECT id, name FROM Playlist WHERE userId = ? AND name = ? OR id LIKE ?',
      [
        ...
        db.serialize(() => {
          ...
          // If the user's active playlist was deleted, try to set another playlist as active
          db.get('SELECT id FROM Playlist WHERE userId = ? LIMIT 1', [ message.author.id ], (err, playlistRow) => {
            db.run('UPDATE User SET activePlaylistId = ? WHERE id = ? AND activePlaylistId = ?',
            [
              (playlistRow && playlistRow.id) ? playlistRow.id : null,
              message.author.id,
              row.id
            ]);
          });
      ...
      db.serialize(function() {
        ...
        db.get('INSERT INTO Playlist (id, userId, name, metadata) VALUES (?, ?, ?, ?)',
          [
            ...
            // Make it the user's active playlist
            db.run('UPDATE User SET activePlaylistId = ? WHERE id = ?', [ playlistId, message.author.id ]);

            message.reply('your playlist **' + playlistName + '** has been created!');
        }); 
      });
    });
  });
...
const getActivePlaylistId = function(userId, callback) {
  log.info('getting active playlist id');
