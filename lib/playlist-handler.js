      ...
      function(err, rows) {
        if (rows && rows.length > 0) {
          ...
          for (const row of rows) {
            if (row.active == 'true') description += '**';
  ...
  db.serialize(function() {
    db.get('INSERT INTO Playlist (id, userId, name, metadata) VALUES (?, ?, ?, ?)',
      [
        ...
        if (err) {
          if (err.code == 'SQLITE_CONSTRAINT') return message.reply(`you already have a playlist named **${playlistName}**. Please choose a different name.`);
