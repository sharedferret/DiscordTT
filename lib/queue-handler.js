...
const cycleQueue = function(userId) {
  db.serialize(function() {
    db.get('SELECT activePlaylistId FROM User u WHERE id = ?', [ userId ], function(err, row) {
      db.serialize(function() {
        db.run('UPDATE UserQueue SET position = (SELECT count(*) + 1 FROM UserQueue WHERE userId = ?) WHERE userId = ? AND playlistId = ? AND position = 1',
          [ userId, userId, row.activePlaylistId ]);
