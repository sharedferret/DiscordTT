/**
 * https://github.com/classdojo/rolling-rate-limiter with minor alterations so that rate-limited
 * requests do not consume a user token.
 */
...
function RateLimiter (options) {
  var redis         = require(global.paths.lib + 'redis-client').client,
    ...
    storeBlocked    = options.storeBlocked;
  
  if (storeBlocked === undefined) storeBlocked = true;
    ...
    return function(id, cb) {
      ...
      batch.exec(function(err, resultArr) {
        ...
        if (tooManyInInterval || timeSinceLastRequest < minDifference) {
          ...
          result = 0;
          if (!storeBlocked) {
            const addKeyBatch = redis.multi();
            addKeyBatch.zadd(key, now, uuid());
            addKeyBatch.expire(key, Math.ceil(interval / 1000000)); // convert to seconds, as used by redis ttl.
            addKeyBatch.exec(function(err, resultArr) {
              if (err) return cb(err);
            });
          }
        }
