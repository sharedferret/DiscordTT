...
var handleYtdlInfo = function(bot, guildId, info, format) {
  ...
  try {
    console.log('\nStream data');
    if (info) {
      console.log('- Relative loudness: ' + info.relative_loudness);
      console.log('- Loudness: ' + info.loudness);
    }

    var intendedVolume = 13 + parseFloat(info.loudness);
    intendedVolume = -13 - intendedVolume;

    console.log('playing stream @ ' + intendedVolume + 'dB');
...
var handleYtdlProgress = function(bot, chunk, downloaded, downloadLength) {
  this.networkUsage.dataIn += chunk;
};

var handleYtdlResponse = function(bot, response) {
