...
const removeDjRequest = (bot, message) => {
  ...
  const removed = _.remove(state[guildId].waitingDjs, function(i) { return i.id == message.author.id; });
...
const playNextSong = (bot, userId, guildId) => {
  ...
  if (userId === bot.user.id) {
    ...
    if (member) {
      ...
      state[guildId].textChannel.send(state[guildId].djs[djIndex].username + '\'s song was skipped since they weren\'t in the voice channel.');
      removeDj(guildId, djIndex);
